# agentinsight-researcher 离线部署脚本
...
Write-Host "========== AgentInsight Researcher 离线部署 ==========" -ForegroundColor Cyan
...
if (-not $SkipCopyModels) {
    ...
    if (Test-Path "packages/models/bge-large-zh-v1.5") {
        ...
        docker run -d --name emb-copier -v agentinsight-researcher_embeddings_models:/data python:3.12-slim sleep 3600 2>&1 | Out-Null
    ...
    if (Test-Path "packages/models/bge-reranker-v2-m3") {
        ...
        docker run -d --name rer-copier -v agentinsight-researcher_rerank_models:/data python:3.12-slim sleep 3600 2>&1 | Out-Null
...
# ========== 3. 构建 agent 镜像 (离线) ==========
Write-Host "[3/4] 构建 agent 镜像 (离线模式)..." -ForegroundColor Yellow
if ($Rebuild) {
    docker compose build --no-cache agent 2>&1 | ForEach-Object { Write-Host "  $_" }
} else {
    docker compose build agent 2>&1 | ForEach-Object { Write-Host "  $_" }
}
Write-Host "  agent 镜像构建完成." -ForegroundColor Green

# ========== 4. 起栈 ==========
Write-Host "[4/4] 启动容器栈..." -ForegroundColor Yellow
...
if ($healthy) {
    ...
    Write-Host "部分容器未健康,请检查 docker compose ps" -ForegroundColor Red
}
