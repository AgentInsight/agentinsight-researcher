...
flowchart LR
    ...
    EMB["Embeddings TEI<br/>bge-base-zh-v1.5 (768维) :8088"]
    RER["Rerank TEI (可选)<br/>bge-reranker-v2-m3 :8089"]
    SX["SearXNG<br/>元搜索引擎 :8099"]
...
- **Embeddings / Rerank** — HuggingFace TEI 服务：`bge-base-zh-v1.5`（中文最强开源嵌入，768 维，用于 Qdrant 索引）+ `bge-reranker-v2-m3`（可选，中文 Rerank SOTA）
- **本地 FastEmbed** — `bge-small-zh-v1.5`（512 维，用于上下文压缩，无需 TEI 服务，本地推理零延迟）
- **元搜索** — SearXNG（自托管元搜索引擎，聚合 Google/Bing/DuckDuckGo 等，端口 8099）
...
│   ├── embeddings.py       # Embeddings 客户端 (TEI 远程 bge-base-zh-v1.5 768维 + FastEmbed 本地 bge-small-zh-v1.5 512维)
...
│   └── ip_user_resolver.py # IP-based 用户身份解析 (JWT 降级兜底)
├── config/         # 配置 SSOT
...
sequenceDiagram
    ...
    MW->>MW: JWT 解析 user_id<br/>(失败降级 IP-based 解析)
    MW->>MW: 安全响应头注入
...
| `user_id` | JWT Bearer Token 解析（失败降级 IP-based 解析） | 用户私有数据隔离 |
| `session_id` | `= thread_id`，请求上下文注入 | 会话级状态隔离（Checkpointer） |

> **用户身份解析双路径**：JWT Bearer Token 为首选路径（经 `GET https://agentinsight.goldebridge.com/api/user` 获取 `user_id`）；JWT 不存在或解析失败时，经 `src/api/ip_user_resolver.py` 基于客户端 IP 生成稳定 `user_id`（IP-based 兜底）。
...
| `searxng` | 本仓 `Dockerfile.searxng`（Alpine + curl_cffi） | 8099 | `wget http://127.0.0.1:8099/` | 仅本机 |
...
- **依赖顺序**（`depends_on: service_healthy`）：`postgres` → `redis` → `qdrant` → `embeddings` → `searxng` → `agent`（`rerank` 可选，`agent` 不强制依赖）
...
- **Embeddings / Rerank** — HuggingFace TEI: `bge-base-zh-v1.5` (768-dim, for Qdrant indexing) + `bge-reranker-v2-m3` (optional)
- **Local FastEmbed** — `bge-small-zh-v1.5` (512-dim, for context compression, no TEI service needed)
- **Meta Search** — SearXNG (self-hosted meta search engine, port 8099)
...
- `user_id` (resolved from JWT Bearer Token, degrades to IP-based resolution via `src/api/ip_user_resolver.py`)
...
Three build modes: QA offline / production online / production offline. Seven-container stack: `agent` / `embeddings` / `rerank` (optional) / `qdrant` / `redis` / `postgres` / `searxng`. Dependency order with `service_healthy` conditions. Agent container initializes PostgreSQL business tables and Qdrant collections on startup (idempotent). APIKey authentication for Qdrant and TEI services via environment variables only.
