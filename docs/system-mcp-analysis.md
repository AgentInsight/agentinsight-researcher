...
**已实现抓取器（9 个，[scrapers/](src/skills/researcher/scrapers)）**：playwright、trafilatura、bs_markdownify、beautiful_soup、arxiv、pymupdf、firecrawl、tavily_extract、markitdown。
...
> 本团队为虚拟专家团，对标项目 AGENTS.md 开发角色，针对系统 MCP 目录治理形成独立立场。
...
2. **MCP 与 searcher/scraper 双轨冗余**：项目已用原生 httpx 实现 22 个 searcher + 9 个 scraper，又在 MCP 目录重复暴露 brave-search/tavily/exa/duckduckgo/arxiv/pubmed/semantic-scholar/firecrawl/playwright 等。这违反 AGENTS.md 第 3 章"依赖单向向内、共享逻辑下沉到 common/"——同一能力两套实现，维护成本翻倍。MCP 应补位而非重复原生能力。
