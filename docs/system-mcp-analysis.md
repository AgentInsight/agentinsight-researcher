...
> **文档版本**：v1.0
> **生成日期**：2026-07-05
> **评估方法**：12 位 AI 专家虚拟团队 × 3 轮多轮讨论 + 项目代码静态审计 + AGENTS.md 合规对标
...
agentinsight-researcher 是以 LangGraph 为编排内核、MCP 为工具协议、AgentInsight SDK 为可观测底座的企业级研究型 Agent（对标 GPT Researcher）。项目通过 [scripts/init.sql](scripts/init.sql) 在 PostgreSQL `mcp_configs` 表中预置了一批 `is_system=TRUE` 的系统公用 MCP 服务，作为用户可克隆的"目录"。
...
3. **重大功能冗余**：项目 [searchers/](src/skills/researcher/searchers) 已实现 15 个搜索引擎（bocha/tavily/brave/bing/google/serper/serpapi/searchapi/searx/exa/duckduckgo/arxiv/pubmed/semantic_scholar/openalex），[scrapers/](src/skills/researcher/scrapers) 已实现 8 个抓取器（firecrawl/playwright/nodriver/beautiful_soup/tavily_extract/markitdown/pymupdf/arxiv）。系统 MCP 中至少 **12 个**与现有原生实现功能重叠，作为 MCP 暴露属重复造轮，且 MCP 经 stdio 子进程通信延迟高于原生 httpx 异步调用。

4. **合规冲突**：[AGENTS.md](AGENTS.md) 第 1/4 章"不推荐清单"明确 MySQL/Memcached/Pinecone/直接调用厂商 SDK（如 openai）等不推荐。系统 MCP 目录中 mysql、mariadb、pinecone、openai 等 7 个与该清单直接冲突，作为"官方目录"预置会传递错误信号。
...
- **轨道 C（合规对标）**：逐条比对 [AGENTS.md](AGENTS.md) 第 1/4/7/9/11/12 章的"优先选择/不推荐/硬约束"清单。
...
| **适合性** | 与项目定位匹配度、与 AGENTS.md 合规一致性 | 5=完全契合；3=中性；1=与"不推荐清单"冲突或破坏架构边界 |
...
**已实现搜索引擎（15 个，[searchers/](src/skills/researcher/searchers)）**：bocha、tavily、brave、bing、google、serper、serpapi、searchapi、searx、exa、duckduckgo、arxiv、pubmed、semantic_scholar、openalex。

**已实现抓取器（8 个，[scrapers/](src/skills/researcher/scrapers)）**：firecrawl、playwright、nodriver、beautiful_soup、tavily_extract、markitdown、pymupdf、arxiv。

> 凡系统 MCP 与上述原生实现重叠者，作为 MCP 暴露均属**冗余**——原生 httpx 异步调用延迟低于 stdio 子进程 + npx 启动开销，且无 MCP 协议序列化损耗。

### 3.3 AGENTS.md 合规红线对照

| AGENTS.md 条款 | 冲突的系统 MCP |
|---|---|
...
> 本团队为虚拟专家团，对标项目 [REQUIREMENTS.md](docs/REQUIREMENTS.md) 开发角色，针对系统 MCP 目录治理形成独立立场。

### 4.1 首席架构师

**立场倾向**：激进精简目录，消除冗余。

1. **目录膨胀破坏架构边界**：130 个系统 MCP 远超研究型 Agent 的合理工具面。项目定位是"深度研究分析"（对标 GPT Researcher），GPT Researcher 的 `mcp/` 模块仅维护十余个工具。当前目录含 shopify/hubspot/spotify/twilio 等明显偏离研究场景的服务，应清理。
2. **MCP 与 searcher/scraper 双轨冗余**：项目已用原生 httpx 实现 15 个 searcher + 8 个 scraper，又在 MCP 目录重复暴露 brave-search/tavily/exa/duckduckgo/arxiv/pubmed/semantic-scholar/firecrawl/playwright 等。这违反 AGENTS.md 第 3 章"依赖单向向内、共享逻辑下沉到 common/"——同一能力两套实现，维护成本翻倍。MCP 应补位而非重复原生能力。
3. **memory/postgres/redis/sqlite MCP 绕过项目基础设施层**：项目 memory/、rag/、config/ 已统一管理 Postgres/Redis/Qdrant。这些 MCP 让 LLM 直接操作基础设施，破坏 AGENTS.md 第 5 章"节点纯函数、状态从 state/deps 获取"原则，应从目录移除。
...
1. **68 个 MCP 需第三方 API Key，数据出境合规风险高**：env_vars 含 `<your-token>` 占位符的 MCP 中，pinecone/replicate/openai/stability/ stripe/coinbase/twilio/sendgrid 等均为境外 SaaS，调用时用户数据出境。AGENTS.md 第 11 章规定"PII 加密存储 + 日志脱敏"，但 MCP 工具调用经 `langchain-mcp-adapters` 透传，[mcp_coordinator.py](src/skills/researcher/mcp_coordinator.py) 未对工具入参/出参做 PII 脱敏，存在违规。
2. **gmail/email-imap/1password/bitwarden 涉及凭据与 PII**：邮件内容、密码库凭据属高敏感 PII。将其作为系统目录预置，诱导用户克隆后让 LLM 读取邮件/密码，违反 AGENTS.md 第 11 章"最小化收集"原则。应移除或至少标注"高风险，需独立审计"。
...
2. **离线部署的 npx 缓存问题**：AGENTS.md 第 12 章要求 QA/离线模式"部署时不联网拉镜像/装依赖"。但 `npx -y <pkg>` 首次运行会从 npm registry 下载包，离线环境直接失败。即便预下载到 npm cache，130 个包的体积与维护成本不可接受。
...
1. **研究型 Agent 的核心数据源应"少而精"**：深度研究需要的是权威信源（学术/新闻/官方报告），而非 130 个泛工具。GPT Researcher 的 retriever 仅约 20 个。当前目录含 figma/spotify/shopify/trello 等明显非研究信源，应清理以降低 LLM 工具选择噪音（`mcp_max_tools=3`，目录越大选择越易出错）。
...
3. **工具调用结果在前端的展示需折叠**：AGENTS.md 第 14 章要求"工具调用展示折叠面板"。MCP 工具结果可能很长（如 elasticsearch 查询返回大 JSON），前端应默认折叠 + 仅展示摘要 + 点击展开。
...
3. **行业适配应利用 MCP 层（AGENTS.md 第 7 章 GPTR 4 层机制）**：MCP 是行业适配的第 4 层。金融研究可推荐 alpha-vantage，电商研究可推荐 shopify（只读）。目录应支持"行业推荐组合"预设，而非让用户从 130 个里自己挑。
...
2. **国内搜索信源缺失**：项目原生 searcher 已有博查（Bocha，国内主搜索），但 MCP 目录无国内搜索（如百度/搜狗）。鉴于 AGENTS.md 第 7 章"中文优先"，MCP 目录应补位国内信源（如百度百科 MCP），但需验证包可用性。
...
4. **合规冲突项（7 个）一律移除**，AGENTS.md "不推荐清单"冲突的不作为官方目录。
...
| 32 | mysql | 5 | 1 | 1 | 🔴 移除 | AGENTS.md 不推荐 MySQL |
...
| 69 | mariadb | 4 | 1 | 1 | 🔴 移除 | AGENTS.md 不推荐 MySQL 系 |
| 70 | qdrant-mcp | 4 | 1 | 1 | 🔴 移除 | 绕过项目 Qdrant 统一管理 |
| 71 | chromadb | 4 | 2 | 3 | 🟡 可选 | 与项目 Qdrant 定位重叠 |
| 72 | pinecone | 4 | 1 | 1 | 🔴 移除 | AGENTS.md 不推荐 Pinecone |
...
| 75 | openai | 5 | 1 | 1 | 🔴 移除 | AGENTS.md 不推荐直连厂商 SDK，项目已用 LiteLLM |
...
治理后目录将从 130 精简至 93（移除 37），其中 18 个"核心保留"作为研究场景的推荐组合，22 个"推荐"作为按需扩展，53 个"可选"作为长尾覆盖。这一精简使 LLM 工具选择面从 130 收窄至合理的可用集合，降低选择噪音与维护成本，同时消除与项目原生 searcher/scraper 的冗余及与 AGENTS.md "不推荐清单"的冲突。
...
## 附录 C：与 AGENTS.md 不推荐清单冲突的 7 个 MCP

| MCP | 冲突条款 | AGENTS.md 出处 |
|---|---|---|
