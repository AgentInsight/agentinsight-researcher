...
7. **语言一致性 (最高优先级硬要求)**: 报告标题、各级标题与正文必须使用单一统一语言撰写, 严禁中英文混合, 严禁整段切换到另一语言. 默认语言为中文; 若 prompt 末尾出现显式语言指令 (如 "Please write the report in English"), 必须以该指令为准用对应语言撰写全文, 原始研究问题的语言不决定报告语言. 上下文中的外语术语可在首次出现时附译法 (如 "立方星 (CubeSat)" 或 "CubeSat (立方星)"), 不得整段保留外语原文.
...
3. **语言一致性 (最高优先级硬要求)**: 子主题语言必须与报告输出语言一致, 子主题语言决定后续章节标题与正文语言, 必须严格一致. 默认输出语言为中文 (子主题用中文短语); 若 prompt 末尾出现显式语言指令 (如 "Please write the report in English"), 必须以该指令为准用对应语言生成子主题. 原始研究问题的语言不决定子主题语言, 仅作为内容参考.
4. 子主题之间应互斥互补, 避免内容重叠
...
10. **语言一致性 (最高优先级硬要求)**: 引言正文必须使用单一统一语言撰写, 严禁中英文混合, 严禁整段切换到另一语言. 默认语言为中文; 若 prompt 末尾出现显式语言指令 (如 "Please write the report in English"), 必须以该指令为准用对应语言撰写全文, 原始研究问题的语言不决定引言语言. 上下文中的外语术语可在首次出现时附译法, 不得整段保留外语原文.
...
8. **语言一致性 (最高优先级硬要求)**: 章节标题 (`## {topic}`) 与所有正文、子小节标题必须使用单一统一语言撰写, 严禁中英文混合, 严禁整段切换到另一语言. 默认语言为中文; 若 prompt 末尾出现显式语言指令 (如 "Please write the report in English"), 必须以该指令为准用对应语言撰写全文 (含章节标题), 原始研究问题的语言不决定章节语言. 若子主题 `{topic}` 语言与目标语言不一致, 必须在章节标题处翻译为目标语言. 上下文中的外语术语可在首次出现时附译法, 不得整段保留外语原文.
...
6. 写作风格: {style_desc}
7. **语言一致性 (最高优先级硬要求)**: 结论正文必须使用单一统一语言撰写, 严禁中英文混合, 严禁整段切换到另一语言. 默认语言为中文; 若 prompt 末尾出现显式语言指令 (如 "Please write the report in English"), 必须以该指令为准用对应语言撰写全文, 原始研究问题的语言不决定结论语言. 上下文中的外语术语可在首次出现时附译法, 不得整段保留外语原文.
